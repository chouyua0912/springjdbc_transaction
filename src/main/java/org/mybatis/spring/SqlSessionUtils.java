...
public final class SqlSessionUtils {
    ...
    public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, ExecutorType executorType, PersistenceExceptionTranslator exceptionTranslator) {
        ...
        SqlSession session = sessionHolder(executorType, holder);   // 获取绑定的SqlSession,holder必须isSynchronizedWithTransaction
        if (session != null) {
            return session;             // 获取到时候就直接返回了
        }
        ...
        session = sessionFactory.openSession(executorType);         // 获取新的SqlSession

        registerSessionHolder(sessionFactory, executorType, exceptionTranslator, session);  // 注册绑定
    ...
    private static void registerSessionHolder(SqlSessionFactory sessionFactory, ExecutorType executorType,
        ...
        if (TransactionSynchronizationManager.isSynchronizationActive()) {      // 当前线程已经开启了事务同步
            Environment environment = sessionFactory.getConfiguration().getEnvironment();

            if (environment.getTransactionFactory() instanceof SpringManagedTransactionFactory) {   // 用SqlSessionFactoryBean创建时候默认的TransactionFactory
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Registering transaction synchronization for SqlSession [" + session + "]");
                }

                holder = new SqlSessionHolder(session, executorType, exceptionTranslator);          // 创建一个holder
                TransactionSynchronizationManager.bindResource(sessionFactory, holder);             // 注册到同步管理器
                TransactionSynchronizationManager.registerSynchronization(new SqlSessionSynchronization(holder, sessionFactory));   // 注册
    ...
    private static SqlSession sessionHolder(ExecutorType executorType, SqlSessionHolder holder) {
        ...
        if (holder != null && holder.isSynchronizedWithTransaction()) {     // 已经完成事务同步的绑定 line 134
            if (holder.getExecutorType() != executorType) {
                throw new TransientDataAccessResourceException("Cannot change the ExecutorType when there is an existing transaction");
            }

            holder.requested();     // 记录参照次数
     ...
     * If it is not, it closes it, otherwise it just updates the reference counter and      根据是否被Spring管理,确定关闭SqlSession
    ...
    private static final class SqlSessionSynchronization extends TransactionSynchronizationAdapter {
        ...
        public void suspend() {
            if (this.holderActive) {
                ...
                TransactionSynchronizationManager.unbindResource(this.sessionFactory);      // 处理事务挂起时候
            }
        }
        ...
        public void resume() {
            if (this.holderActive) {
                ...
                TransactionSynchronizationManager.bindResource(this.sessionFactory, this.holder);   // 事务继续了
            }
        }
