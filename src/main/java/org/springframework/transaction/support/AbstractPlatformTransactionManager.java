...
/**
 * Abstract base class that implements Spring's standard transaction workflow,  对外提供能力是获取TransactionStatus，commit，rollback
 * serving as basis for concrete platform transaction managers like             TransactionStatus 记录当前事务的状态
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
    ...
    public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
        ...
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {   // 要求上层必须传递下来事务
            ...
            try {
                ...
                doBegin(transaction, definition);   // 实际获取conn连接,开启事务，设置只读，隔离级别。 事务传播是Spring是实现的特性，并不是是数据库特性，无法设置
    ...
    /**
     * Create a TransactionStatus for an existing transaction.  处理已经有线程绑定conn
     */
    private TransactionStatus handleExistingTransaction(
            TransactionDefinition definition, Object transaction, boolean debugEnabled)
            throws TransactionException {

        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {   // 不允许事务传播存在
            throw new IllegalTransactionStateException(
                    "Existing transaction found for transaction marked with propagation 'never'");
        }

        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {   // 不支持事务
            if (debugEnabled) {
        ...
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
            ...
            SuspendedResourcesHolder suspendedResources = suspend(transaction); // 挂起当前事务，降ConnectionHolder设置null，取出当前ConnectionHolder绑定的resource返回
            try {
                boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
                DefaultTransactionStatus status = newTransactionStatus(
                        definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
    ...
    protected void prepareSynchronization(DefaultTransactionStatus status, TransactionDefinition definition) {
        if (status.isNewSynchronization()) {    // 检查是否需要绑定到 线程变量
    ...
    protected final SuspendedResourcesHolder suspend(Object transaction) throws TransactionException {
        if (TransactionSynchronizationManager.isSynchronizationActive()) {  // 需要对绑定在线程的资源进行处理
    ...
    /**
     * This implementation of commit handles participating in existing  首先检查是否需要rollback，然后再进行commit
    ...
    public final void commit(TransactionStatus status) throws TransactionException {
        ...
        if (!shouldCommitOnGlobalRollbackOnly() && defStatus.isGlobalRollbackOnly()) {
            ...
            // or if explicitly asked to.   已经标记为全局需要回滚了 子调用里面 提交事务进行回滚不会实际执行，而是标记全局事务为回滚状态，如果父调用任然commit则会报错
    ...
    private void processCommit(DefaultTransactionStatus status) throws TransactionException {
        try {
            ...
            try {
                ...
                else if (status.isNewTransaction()) {   // 最外层获取到的连接，或者重新获取到的新连接才会去执行实际的commit
                    if (status.isDebug()) {
        ...
        try {
            try {
                ...
                if (status.hasSavepoint()) {
                    ...
                    doRollback(status); // 在创建出来这个conn的transaction里面才做实际的rollback，被调用的方法，复用的conn的只是标记conn为rollback only，等待最顶层的调用去执行实际的rollback
                }
