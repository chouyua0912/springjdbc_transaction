 ...
 */
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {  // 实现类为TransactionInterceptor，通过被代理，支持了Aop操作
    ...
    protected Object invokeWithinTransaction(Method method, Class<?> targetClass, final InvocationCallback invocation)
        ...
        if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
            ...
            try {
                ...
                retVal = invocation.proceedWithInvocation();    // 没有在此处将获取到的conn传递给被增强方法，要求被增强方法获取conn的时候也是从TransactionManger获取
            }
            catch (Throwable ex) {
    ...
    protected TransactionInfo createTransactionIfNecessary(
        ...
        if (txAttr != null) {
            if (tm != null) {
                status = tm.getTransaction(txAttr);     // 根据指定的事务属性，从TransactionManager获取事务
            }
            else {
